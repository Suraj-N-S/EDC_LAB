...
\begin{enumerate}
    \item To construct an N-channel enhancement MOSFET Common-Source (CS) amplifier with a resistive drain load ($R_D$).
...
\midrule
1 & N-Channel MOSFET & 2N7002 / 2N7000/N7000 & 1 \\
...
\subsection{Device Threshold Voltage (\texorpdfstring{$V_T$}{VT}) from Experiment 3 Transfer Data}
To identify the threshold voltage ($V_T$) of the NMOS transistor used in this circuit, the transfer characteristic data ($I_D$ vs $V_{GS}$) measured at $V_{DS} = 5\text{ V}$ from Experiment~3 Part~B is incorporated below in Table~\ref{tab:exp3_transfer}.

\begin{table}[H]
\centering
\caption{Experiment 3 (Part B): NMOS Transfer Characteristic at $V_{DS} = 5\text{ V}$}\label{tab:exp3_transfer}
\begin{tabular}{cccc}
\toprule
\textbf{S.No.} & \textbf{Gate-Source Voltage $V_{GS}$ (V)} & \textbf{Drain Current $I_D$ ($\mu$A)} & \textbf{$\sqrt{I_D}$ ($\sqrt{\mu\text{A}}$)} \\
\midrule
1  & 0.0 & 0.0    & 0.000 \\
2  & 1.1 & 0.1    & 0.316 \\
3  & 1.2 & 0.4    & 0.632 \\
...
8  & 1.7 & 615.5  & 24.809 \\
9  & 1.8 & 1762.0 & 41.976 \\
10 & 1.9 & 4250.0 & 65.192 \\
11 & 2.0 & 8760.0 & 93.595 \\
12 & 2.1 & 122800.0 & 350.428 \\
\bottomrule
\end{tabular}
\end{table}

\noindent \textbf{Threshold Voltage Determination from $V_{DS} = 5\text{ V}$ Readings:}
\begin{itemize}
    \item \textbf{Subthreshold vs Strong Inversion:} In the range $V_{GS} \le 1.3\text{ V}$, the drain current is virtually negligible ($I_D \le 1.9\ \mu\text{A}$), corresponding to the subthreshold/weak inversion regime. At $V_{GS} = 1.4\text{ V}$, the current exhibits a sharp rise to $9.3\ \mu\text{A}$, accelerating rapidly to $43.2\ \mu\text{A}$ at $1.5\text{ V}$ and $178.6\ \mu\text{A}$ at $1.6\text{ V}$, marking the onset of strong channel inversion.
    \item \textbf{Square-Root Characteristic ($\sqrt{I_D}$ vs $V_{GS}$) Extrapolation:} In saturation, $I_D \propto (V_{GS} - V_T)^2 \implies \sqrt{I_D} \propto (V_{GS} - V_T)$. Linearly extrapolating $\sqrt{I_D}$ from the onset of strong inversion between $V_{GS} = 1.4\text{ V}$ and $1.5\text{ V}$:
    \begin{equation}
        V_T = V_{GS1} - \sqrt{I_{D1}} \cdot \frac{V_{GS2} - V_{GS1}}{\sqrt{I_{D2}} - \sqrt{I_{D1}}} = 1.40 - 3.050 \cdot \frac{1.50 - 1.40}{6.573 - 3.050} \approx 1.40 - 0.086 = 1.314\text{ V}
    \end{equation}
    Similarly, evaluating between $1.5\text{ V}$ and $1.6\text{ V}$ yields:
    \begin{equation}
        V_T = 1.50 - 6.573 \cdot \frac{1.60 - 1.50}{13.364 - 6.573} = 1.50 - 0.097 = 1.403\text{ V} \approx 1.4\text{ V}
    \end{equation}
    This confirms that the device enters strong inversion saturation at \textbf{$V_T \approx 1.4\text{ V}$}.
\end{itemize}
...
7  & 1.20 & 9.986 & Cut-off \\
8  & 1.25 & 9.980 & Cut-off / Weak Inversion \\
9  & 1.30 & 9.970 & Cut-off / Subthreshold \\
10 & 1.35 & 9.930 & Near Threshold ($V_{\text{IN}} \approx V_T$) \\
\midrule
11 & 1.40 & 9.860 & Saturation (Onset of Strong Inversion Conduction) \\
...
\textbf{Note : }This plot matches with the plot of VOUT vs VIN given in the assignment pdf.

\subsection{Deductions and Region Boundaries (Part A)}
From the measured transfer characteristic:
\begin{itemize}
    \item \textbf{Cut-off Region ($0.00\text{ V} \le V_{\text{IN}} < 1.40\text{ V}$):} The output remains virtually constant at $V_{\text{OUT}} \approx 9.99\text{ V} \approx V_{DD}$ (with negligible current draw through $R_D = 10\text{ k}\Omega$). Conduction is negligible because the gate potential is insufficient to create a strong inversion layer.
    \item \textbf{Active Saturation Region ($1.40\text{ V} \le V_{\text{IN}} \le 1.70\text{ V}$):} Beyond threshold, significant channel current $I_D$ turns on. For instance, at $V_{\text{IN}} = 1.70\text{ V}$, the overdrive voltage with $V_T \approx 1.40\text{ V}$ is $V_{GS} - V_T \approx 1.70 - 1.40 = 0.30\text{ V}$. Since $V_{\text{OUT}} = 2.21\text{ V} > 0.30\text{ V}$, the condition $V_{DS} \ge V_{GS} - V_T$ is strictly satisfied. This constitutes the high-slope amplification region where $V_{\text{OUT}}$ drops steeply from $9.86\text{ V}$ down to $2.21\text{ V}$.
    \item \textbf{Triode Boundary and Ohmic Region ($V_{\text{IN}} \ge 1.75\text{ V}$):} At $V_{\text{IN}} = 1.75\text{ V}$, $V_{\text{OUT}}$ plummets to $0.09\text{ V}$, which is far below $V_{GS} - V_T \approx 0.35\text{ V}$. The transistor enters triode, and further increases in $V_{\text{IN}}$ pull $V_{\text{OUT}}$ asymptotically to nearly $0\text{ V}$ ($0.01\text{ V}$ to $0.00\text{ V}$).
\end{itemize}
...
\begin{enumerate}
    ...
    The AC output voltage was measured as an RMS value ($V_{\text{OUT(RMS)}}$) using DMM. Since the applied AC input had a peak amplitude $v_{in,\text{peak}} = 10\text{ mV}$, its corresponding RMS value is $v_{in,\text{rms}} = \frac{10\text{ mV}}{\sqrt{2}} \approx 7.07\text{ mV}$. The peak AC output voltage is $v_{out,\text{peak}} = \sqrt{2} \cdot V_{\text{OUT(RMS)}} = 1.414 \cdot V_{\text{OUT(RMS)}}$. Taking into account the intrinsic $180^\circ$ phase inversion of the common-source configuration, the small-signal voltage gain is:
    \begin{equation}
...
\textbf{NOTE : } We could not obtain the output on DSO because the DSO could not pickup the voltages as low as 10-100 mV. It was giving very noisy output. Hence, we measured the RMS values of the output sin waveforms using Multimeter.
\begin{table}[H]
\centering
...
7  & 1.20 & 62.00 & -8.77  & Cut-off \\
8  & 1.30 & 85.00 & -12.02 & Subthreshold \\
9  & 1.35 & 150.00 & -21.21 &Threshold Transition\\
10 & 1.40 & 310.00 & -43.83 & Saturation ($V_{\text{IN}} \ge V_T$) \\
11 & 1.45 & 640.00 & \textbf{-90.50} & \textbf{Maximum Gain Peak} \\
12 & 1.50 & 7.20   & -1.02  & Transition \\
...
\textbf{Note : }This plot matches with the plot of Gain vs VIN given in the assignment pdf.

\newpage

\subsection{Comparison: Method 1 (DC Slope) vs. Method 2 (AC Gain)} 

\textbf{Note : The $R_D$ used in Method 1 and Method 2 are different.\\
In Method 1 : $R_D = 10k \ohm$ and in Method 2 : $R_D = 300k \ohm$ .\\
In Mehtod 2 the value of $R_D$ was taken to be a higher value to perform the experiment with suitable results.}
\\\\
...
\toprule
\textbf{Bias $V_{\text{IN}}$ (V)} & \textbf{$V_{\text{OUT,DC}}$ (V)} & \textbf{Method 1: $\frac{dV_{\text{OUT}}}{dV_{\text{IN}}}$} & \textbf{$V_{\text{OUT(RMS)}}$ (mV)} & \textbf{Method 2: $A_{v,\text{AC}}$} & \textbf{Note} \\
\midrule
0.00 & 9.99 & 0.00   & 62.00  & -8.77  & Cut-off\\
0.60 & 9.99 & 0.00   & 62.00  & -8.77  & Cut-off\\
1.20 & 9.99 & -0.04  & 62.00  & -8.77  & Cut-off\\
1.30 & 9.97 & -0.50  & 85.00  & -12.02 & Subthreshold conduction; slight AC response \\
1.35 & 9.93 & -1.10  & 150.00 & -21.21 & Device approaching threshold ($V_{\text{IN}} \to V_T$) \\
1.40 & 9.86 & -2.10  & 310.00 & -43.83 & Strong inversion ($V_{\text{IN}} \ge V_T \approx 1.4\text{ V}$); rapid $g_m$ surge \\
...
1.60 & 7.65 & -32.10 & 0.83   & -0.12  & Triode region\\
...
1.80 & 0.05 & -0.60  & 0.00   & 0.00   & Deep triode\\
\bottomrule
\end{tabular}
\end{table}

\subsubsection*{Comparison between Method and Method 2}
\begin{enumerate}
    \item \textbf{Cut-off and Noise Floor / Feedthrough ($V_{\text{IN}} \le 1.20\text{ V}$):}
    \begin{itemize}
        \item \textbf{Method 1:} The DC transfer curve is flat ($V_{\text{OUT}} \approx 9.99\text{ V}$), yielding a numerical slope $\frac{dV_{\text{OUT}}}{dV_{\text{IN}}} \approx 0.00$. This reflects true zero channel transconductance ($g_m = 0$).
        \item \textbf{Method 2:} Measured an RMS output floor of $V_{\text{OUT(RMS)}} = 62.00\text{ mV}$ ($A_{v,\text{AC}} = -8.77$). Because the device is off, this is not true active transistor amplification; rather, it stems from capacitive AC feedthrough via gate-drain parasitic capacitance.
    \end{itemize}

    \item \textbf{Discrepancy in Gain Magnitude and Peak Location due to Load Resistance ($R_D$):}
    \begin{itemize}
        \item In Method 1 (Part A), a load resistor $R_D = 10\text{ k}\Omega$ was used, causing the device to enter triode much later ($V_{\text{IN}} \approx 1.75\text{ V}$). The numerical slope peak reaches $|A_v| \approx 55.00$ to $67.6$ near $V_{\text{IN}} \approx 1.68\text{--}1.70\text{ V}$.
        \item In Method 2 (Part B), a $30\times$ larger load resistor $R_D = 300\text{ k}\Omega$ was used. Since small-signal gain scales with $g_m R_D$, the measured gain peaks at a significantly higher value of \textbf{$|A_{v,\text{AC}}| = 90.50$} at \textbf{$V_{\text{IN}} = 1.45\text{ V}$}. The much larger $R_D$ creates a much larger $I_D R_D$ drop for the same overdrive, shifting the saturation-to-triode boundary to a substantially lower gate voltage ($V_{\text{IN}} \approx 1.50\text{ V}$).
    \end{itemize}

    \item \textbf{Subthreshold and Threshold Transition ($V_{\text{IN}} = 1.30\text{ V}$ to $1.45\text{ V}$):}
    \begin{itemize}
        \item Both methods show a sharp, monotonic increase in gain as the transistor turns on past $V_T \approx 1.40\text{ V}$.
        \item At $V_{\text{IN}} = 1.35\text{ V}$ and $1.40\text{ V}$, Method 1 shows slopes of $-1.10$ and $-2.10$ respectively, while Method 2 records gains of $-21.21$ and $-43.83$. The approximately $20\times$ to $30\times$ ratio between Method 2 and Method 1 directly aligns with the $30\times$ ratio of their drain load resistances ($300\text{ k}\Omega / 10\text{ k}\Omega = 30$).
    \end{itemize}

    \item \textbf{Large-Signal Output Clipping and Triode Collapse ($V_{\text{IN}} \ge 1.50\text{ V}$):}
    \begin{itemize}
        \item In Method 1 ($R_D = 10\text{ k}\Omega$), the transistor remains in saturation through $V_{\text{IN}} = 1.70\text{ V}$, maintaining high DC slope magnitudes ($-9.20$ to $-55.00$).
        \item In Method 2 ($R_D = 300\text{ k}\Omega$), the DC output for $V_{\text{IN}} \ge 1.50\text{ V}$ swings into the knee/triode region. With $A_v \approx 90$, the $10\text{ mV}$ peak AC input produces large output swings that severely clip against ground. Consequently, the small-signal assumption ($v_{in} \ll 2I_D/g_m$) breaks down, and the DMM records crushed RMS fundamental values ($7.20\text{ mV} \to 0.00\text{ mV}$), giving an apparent collapse of gain ($A_v \to 0$).
    \end{itemize}
\end{enumerate}
...
\toprule
\textbf{Parameter} & \textbf{(Manual Sec. 6)} & \textbf{Experimental} & \textbf{Difference} \\
\midrule
Drain Resistor & $4.7\text{ k}\Omega$ & $10\text{ k}\Omega$ / $300\text{ k}\Omega$ & Differing load designs \\
Threshold Voltage & $2.00\text{ V}$ & \textbf{$\approx 1.40\text{ V}$} & \textbf{$-0.60\text{ V}$ (Lower by $30\%$)} \\
Gain Peak Location & $\approx 3\text{ V}$ & \textbf{$1.45\text{V}$ (B)/$1.68\text{V}$(A)} & \textbf{$-1.30\text{V}$/$-1.07\text{V}$ (Shifted Left)} \\
\bottomrule
\end{tabular}
\end{table}
\begin{itemize}
    \item \textbf{Direction of Difference:} Both $V_T$ and the peak gain bias point $V_{\text{IN,peak}}$ for our MOSFET are significantly \textbf{lower} (shifted to the left on the $V_{\text{IN}}$ axis) compared to the illustrative Section 6 manual values.
    \item \textbf{Physical Reason:} The actual device has an experimentally characterized threshold voltage of $V_T \approx 1.40\text{ V}$ (extrapolated from Experiment~3 Part~B transfer data at $V_{DS} = 5\text{ V}$, where strong inversion initiates between $1.4\text{ V}$ and $1.5\text{ V}$), which is lower than the hypothetical $2.00\text{ V}$ in Section 6. Furthermore, because our load resistance is higher ($10\text{ k}\Omega$ and $300\text{ k}\Omega$ versus $4.7\text{ k}\Omega$), the output voltage drops rapidly, reaching the saturation-triode boundary at a lower gate bias ($1.45\text{ V}$--$1.68\text{ V}$).
\end{itemize}
...
\begin{enumerate}
    \item The DC transfer characteristic ($V_{\text{OUT}}$ vs $V_{\text{IN}}$) of an N-channel enhancement MOSFET Common-Source amplifier was experimentally measured, verifying the three distinct operational domains: \textbf{Cut-off} ($V_{\text{IN}} < 1.40\text{ V}$, $V_{\text{OUT}} \approx 9.99\text{ V}$), \textbf{Saturation} ($1.40\text{ V} \le V_{\text{IN}} \le 1.70\text{ V}$, active amplification and onset of strong inversion with $V_T \approx 1.40\text{ V}$), and \textbf{Triode} ($V_{\text{IN}} \ge 1.75\text{ V}$, $V_{\text{OUT}} \approx 0.09\text{--}0\text{ V}$).
    \item Threshold voltage verification using Experiment~3 Part~B transfer measurements ($I_D$ vs $V_{GS}$ at $V_{DS} = 5\text{ V}$) confirmed that strong inversion initiates at $V_T \approx 1.4\text{ V}$ via $\sqrt{I_D}$ extrapolation, directly correlating with the rapid onset of voltage drop and AC gain peak at $V_{\text{IN}} = 1.45\text{ V}$.
    ...
    \end{itemize}
    \item Comprehensive analysis of Section 8 (in the manual) discussion points proved that peak gain occurs at maximum transconductance ($g_m$) right near the edge of saturation, while cutoff and deep triode exhibit zero incremental gain.
\end{enumerate}
