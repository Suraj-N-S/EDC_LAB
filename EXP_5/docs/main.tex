...
As shown in Table 6, there is a significant discrepancy ($\approx 92\%$ reduction) between the theoretical calculations and the measured bench results for both transconductance ($g_m$) and differential voltage gain ($A_d$):
...
Transconductance ($g_m$) & $2509.0\text{ mA/V}$ & $199.1\text{ mA/V}$ & $\approx 92.1\%$ drop \\
Differential Gain ($A_d$) & $451.62\text{ V/V}$ & $35.83\text{ V/V}$ & $\approx 92.1\%$ drop \\ \hline
\end{tabular}
\label{tab:diff_pair_comparison}
\end{table}

\textbf{The primary physical causes for this severe degradation are detailed below}:

\subsection{Local Source Degeneration via Passive Resistor ($R_{SS}$)}
The theoretical subthreshold transconductance model,
\begin{equation}
g_m = \frac{I_{SS}}{n V_T}
\end{equation}
...
As $V_{in+}$ was swept, the common-source node voltage $V_S$ was not fixed, causing substantial local source degeneration. The effective AC transconductance $g_{m,eff}$ is given by:
\begin{equation}
g_{m,eff} = \frac{g_m}{1 + g_m R_{SS}}
\end{equation}
Substituting $g_m \approx 2509\text{ mA/V}$ and $R_{SS} = 22\,\Omega$:
\begin{equation}
1 + g_m R_{SS} = 1 + (2.509)(22) \approx 56.2
\end{equation}
