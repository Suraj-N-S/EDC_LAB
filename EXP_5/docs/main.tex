...
\begin{enumerate}
    ...
    \item To establish, verify, and document the DC operating (quiescent) point by adjusting the common-mode gate bias such that a nominal tail bias current $I_{SS} \approx 80\text{ mA}$ is achieved with $V_{DD} = 12\text{ V}$, $R_D = 180\ \Omega$, and $R_{SS} = 22\ \Omega$.\\\textbf{Note:} These values of resistances were chosen based on availability in lab.
...
\end{equation}
The differential small-signal transconductance (differential transconductance of the pair) and differential voltage gain are:
\begin{equation}
    g_m = \frac{I_{SS}}{2 n V_T}, \qquad A_d = g_m R_D
\end{equation}
...
\noindent\textbf{Key Observation on Device Matching:} Transistor $M_1$ turns on sharply around \textbf{$V_{GS} \approx 0.60\text{--}0.61\text{ V}$}, whereas $M_2$ exhibits a lower subthreshold knee around \textbf{$0.48\text{--}0.49\text{ V}$}. \textbf{This slight physical threshold offset ($\Delta V_{th} \approx 100\text{ mV}$) accounts for the minor quiescent drain voltage imbalance ($V_{D1} \neq V_{D2}$) observed in subsequent sections}.
...
\begin{itemize}
    ...
    \item \textbf{Measured Quiescent Tail Current:} $I_{S,Q} = 80.1\text{ mA}$ ($V_{RSS} = 1.762\text{ V}$)
...
\begin{figure}[H]
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        ...
        \caption{LTspice Simulation: $I_{S}$ vs. $V_{cm}$}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.48\textwidth}
...
\subsection{Experimental Observation Table}
\begin{table}[H]
\centering
...
\\\textbf{At Q-Point : $I_{D1}$ = (12-4.55)/180 = 41.39 mA and $I_{D2}$ = (12-4.66) = 40.78 mA.\\
$I_{D1} \approx I_{D2}$}.
\newpage
...
\begin{figure}[H]
    \centering
    \begin{subfigure}[b]{0.48\textwidth}
        \includegraphics[width=\textwidth]{../SIM/7.2/sim_vss.png}
        \caption{LTspice Simulation: $V_{RSS}$ vs. $V_{cm}$}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.48\textwidth}
        \includegraphics[width=\textwidth]{../PLOTS/CM/vss_vs_vcm.png}
        \caption{Experimental Plot: $V_{RSS}$ vs. $V_{cm}$}
    \end{subfigure}
    \caption{Common-Mode Sweep: Common-Source Node / Tail Voltage Comparison ($V_{RSS}$ vs. $V_{cm}$).}
    \label{fig:cm_vss}
\end{figure}
...
\begin{table}[H]
\centering
\caption{Common-Mode Sweep Observations: $V_{cm}$, Drain Voltages, Tail Voltage $V_{RSS}$, and $I_{S}$ ($R_{SS} = 22\ \Omega$)}
\label{tab:sheet2}
\begin{tabular}{@{}cccccc@{}}
\toprule
\textbf{$V_{cm}$ [V]} & \textbf{$V_{D1}$ [V]} & \textbf{$V_{D2}$ [V]} & \textbf{$V_{RSS}$ [V]} & \textbf{$I_{S} = V_{RSS}/R_{SS}$ [mA]} & \textbf{Region / State} \\ \midrule
...
\begin{enumerate}
    ...
    \end{equation}
    \textbf{The measured average common-mode gain $|A_{cm,\text{meas}}| \approx 3.82\text{ V/V}$ is in close agreement with the theoretical value $A_{cm} \approx -\frac{R_D}{2 R_{SS}} = -\frac{180}{44} = -4.091\text{ V/V}$ (relative error of only $6.6\%$).}
...
\begin{figure}[H]
    \centering
    \begin{subfigure}[b]{0.48\textwidth}
        \includegraphics[width=\textwidth]{../SIM/7.3/sim_vd1_vd2_vs_vid.png}
        \caption{LTspice Simulation: $V_{D1}, V_{D2}$ vs. $V_{id}$}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.48\textwidth}
        \includegraphics[width=\textwidth]{../PLOTS/DIFF_MODE/vd1_vd2_vs_vid.png}
        \caption{Experimental Plot: $V_{D1}, V_{D2}$ vs. $V_{id}$}
    \end{subfigure}
    \caption{Differential-Mode Sweep: Drain Voltage Comparison ($V_{D1}, V_{D2}$ vs. $V_{id}$).}
    \label{fig:diff_vd}
\end{figure}
...
\begin{figure}[H]
    \centering
    \begin{subfigure}[b]{0.48\textwidth}
        \includegraphics[width=\textwidth]{../SIM/7.3/sim_id1_id2_vs_vid.png}
        \caption{LTspice Simulation: $I_{D1}, I_{D2}$ vs. $V_{id}$}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.48\textwidth}
        \includegraphics[width=\textwidth]{../PLOTS/DIFF_MODE/id1_id2_vs_vid.png}
        \caption{Experimental Plot: $I_{D1}, I_{D2}$ vs. $V_{id}$}
    \end{subfigure}
    \caption{Differential-Mode Sweep: Branch Currents Comparison ($I_{D1}, I_{D2}$ vs. $V_{id}$).}
    \label{fig:diff_id}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{../PLOTS/DIFF_MODE/abs_del_id_vs_vid_semilog.png}
...
\end{equation}
At room temperature (T=300K) , $V_{T} = 25.85 mV$.\\
...
\begin{enumerate}
    ...
    \begin{equation}
        g_{m,\text{plan}} = \frac{I_{SS}}{2 n V_T} = \frac{80.1\text{ mA}}{2 \times 1.5 \times 25.85\text{ mV}} = 1032.9\text{ mA/V} = 1.033\text{ A/V}
    \end{equation}
    With the actual drain load used in the lab ($R_D = 180\ \Omega$):
    \begin{equation}
        A_{d,\text{plan}} = g_{m,\text{plan}} R_D = 1.033 \times 180 = \mathbf{185.92\text{ V/V}} \quad (45.39\text{ dB})
    \end{equation}

    \item \textbf{Theoretical Prediction with Measured $n$ ($n_{\text{meas}} = 1.235$):}
    \begin{equation}
        g_{m,\text{thy}} = \frac{I_{SS}}{2 n_{\text{meas}} V_T} = \frac{80.1\text{ mA}}{2 \times 1.235 \times 25.85\text{ mV}} = \mathbf{1254.5\text{ mA/V}} = 1.255\text{ A/V}
    \end{equation}
    \begin{equation}
        A_{d,\text{thy}} = g_{m,\text{thy}} R_D = 1.255 \times 180 = \mathbf{225.81\text{ V/V}} \quad (47.07\text{ dB})
    \end{equation}
    ...
    \end{equation}
    \begin{equation}
        A_{d,\text{meas}} = g_{m,\text{meas}} R_D = 0.1991 \times 180 = \mathbf{35.83\text{ V/V}} \quad (31.09\text{ dB})
    \end{equation}
\end{enumerate}
...
$g_m$ (mA/V) & 1032.9 & 1254.5 & 199.1 \\
$A_d = g_m R_D$ (V/V) & 185.92 & 225.81 & 35.83 \\
\midrule
\multicolumn{2}{l}{\textbf{Percentage Deviation:}} & \multicolumn{2}{c}{$\frac{|g_{m,\text{thy}} - g_{m,\text{meas}}|}{g_{m,\text{thy}}} \times 100\% = \mathbf{84.1\%}$} \\
\bottomrule
\end{tabular}
\end{table} 
...
As shown in Table \ref{tab:diff_pair_comparison}, there is a discrepancy ($\approx 84.1\%$ difference) between the theoretical calculations and the measured bench results for both transconductance ($g_m$) and differential voltage gain ($A_d$):

\begin{table}[h!]
\centering
\caption{Comparison of Theoretical and Measured Differential Pair Parameters}
\begin{tabular}{lcccc}
\hline
\textbf{Parameter} & \textbf{Theoretical ($n_{meas} = 1.235$)} & \textbf{Measured Bench} & \textbf{Deviation / Error} \\ \hline
Transconductance ($g_m$) & $1254.5\text{ mA/V}$ & $199.1\text{ mA/V}$ & $\approx 84.1\%$ drop \\
Differential Gain ($A_d$) & $225.81\text{ V/V}$ & $35.83\text{ V/V}$ & $\approx 84.1\%$ drop \\ \hline
\end{tabular}
\label{tab:diff_pair_comparison}
\end{table}

\textbf{The primary physical causes for this degradation are detailed below}:

\subsection{Local Source Degeneration via Passive Resistor (RSS)}
The theoretical subthreshold differential transconductance model,
\begin{equation}
g_m = \frac{I_{SS}}{2 n V_T}
\end{equation}
\textbf{assumes an ideal, infinite-impedance constant tail current source ($R_{SS} \to \infty$). However, the practical circuit utilized a passive tail resistor $R_{SS} = 22\,\Omega$.} 

As $V_{in+}$ was swept, the common-source node voltage $V_S$ was not fixed, causing substantial local source degeneration. The effective AC transconductance $g_{m,eff}$ is degraded by degeneration and source resistance:
\begin{equation}
g_{m,eff} = \frac{g_m}{1 + g_m R_{SS}}
\end{equation}
Substituting $g_m \approx 1254.5\text{ mA/V} = 1.255\text{ A/V}$ and $R_{SS} = 22\,\Omega$:
\begin{equation}
1 + g_m R_{SS} = 1 + (1.255)(22) \approx 28.6
\end{equation}
\textbf{This degeneration factor accounts for the vast majority of the observed gain and transconductance reduction.}
While Equation~(30) models full single-ended degeneration yielding $g_{m,\text{eff}} = 43.9\text{ mA/V}$, the measured transconductance $g_{m,\text{meas}} = 199.1\text{ mA/V}$ sits between this lower bound and the ideal differential value ($1254.5\text{ mA/V}$). This occurs because holding $V_{\text{in}-}$ fixed while sweeping $V_{\text{in}+}$ introduces a mixed-mode drive, where $Q_2$ provides partial differential feedback through the non-driven side of the pair, thereby mitigating the full degenerating effect of $R_{SS}$.
\subsection{Tail Current Modulation and Dynamic Asymmetry}
During the differential input sweep ($V_{id} \in [-300\text{ mV}, +300\text{ mV}]$), $V_{in-}$ ($V_2$) was held static at $3.08\text{ V}$ while $V_{in+}$ ($V_1$) was varied. Because $R_{SS}$ is small ($22\,\Omega$), altering $V_1$ dynamically shifted the common-source voltage $V_S$, modulating the total tail current $I_{SS}$ across the sweep rather than keeping it constant at $80.1\text{ mA}$. This altered the drain current ($I_D$) slopes relative to theoretical expectations.

\subsection{Moderate Inversion Transition and Parasitics}
At $I_{SS} \approx 80.1\text{ mA}$, the transistors operate near the border of moderate inversion. In this region, the exponential current-voltage relation degrades toward a square-law/linear characteristics, lowering the effective $g_m$. Furthermore, parasitic resistance from breadboard contacts, probe interconnects, and internal MOSFET source resistances contributed additional unintended degeneration.
